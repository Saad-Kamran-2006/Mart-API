...
# Extract the first broker from the KAFKA_HOST environment variable
FIRST_BROKER=$(echo $KAFKA_HOST | cut -d',' -f1 | cut -d':' -f1)

# Wait for Kafka to be ready
while ! nc -z $FIRST_BROKER 19092; do
    echo "Waiting for Kafka to be available on $FIRST_BROKER:19092..."
    sleep 3
done
echo "Kafka is now reachable. Proceeding with topic creation."

# Loop over comma-separated topic definitions and create them
# Expected format: "topicname:partitions:replicationfactor"
IFS=','; for topicToCreate in $KAFKA_CREATE_TOPICS; do
    ...
    # Create the topic
    kafka-topics.sh --create --bootstrap-server $FIRST_BROKER:19092 \
